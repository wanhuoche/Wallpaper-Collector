...
D.ratioTags.addEventListener('click', function(e) {
    ...
    setState('selectedRatio', tag.dataset.ratio);
    W.storage.saveLocal();
...
D.qualityTags.addEventListener('click', function(e) {
    ...
    tag.classList.add('active');
    setState('selectedQuality', tag.dataset.quality);
    W.storage.saveLocal();
    if (W.state.currentQuery) { W.state.currentPage = 1; W.state.allPhotos = []; W.favorites.switchTab('search'); filterSearch(); }
});
...
D.purityTags.addEventListener('click', function(e) {
    ...
    tag.classList.add('active');
    setState('selectedPurity', tag.dataset.purity);
...
if (hideFavedCheckbox) {
    hideFavedCheckbox.addEventListener('change', function() {
        setState('hideFaved', hideFavedCheckbox.checked);
