...
function applySettings(s) {
    if (!s) return;
    if (s.source) setState('source', s.source);
    if (s.apiKeys) {
    ...
    if (s.perPage) setState('perPage', s.perPage);
    if (s.ratioTolerance !== undefined) setState('ratioTolerance', s.ratioTolerance);
