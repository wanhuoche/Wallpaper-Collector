        ...
        try {
            ...
            if (!resp.ok) {
                ...
                if (resp.status === 429) {
                    ...
                    var ctaHtml = '<div class="empty-state rate-limit">'
                        + '<div class="empty-state-icon emoji">⏰</div>'
                        + '<h3 class="empty-state-title">今日搜索次数已用完</h3>'
                        + '<p class="empty-state-desc">' + (errData.error || '请明天再试') + '</p>'
                        + '<div class="empty-state-actions">';
                    if (!isLoggedIn) {
                        ctaHtml += '<a href="login.html" class="btn-cta primary">登录（提升至 40 次/天）</a>';
                    }
                    ctaHtml += '<button onclick="document.getElementById(\'btnSettings\').click()" class="btn-cta secondary">填写自己的 API Key（无限制）</button>'
                        + '</div></div>';
